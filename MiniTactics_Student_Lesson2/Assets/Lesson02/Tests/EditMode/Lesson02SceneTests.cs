...
{
    public sealed class Lesson02SceneTests
    {
        private const string ScenePath = "Assets/Lesson02/Scenes/Lesson02.unity";

        [Test]
        public void Lesson02Scene_ContainsWiredClickMovementBoardUnitsAndCamera()
        {
            ...
            string[] prefabPaths =
            {
                "Assets/Lesson02/Prefabs/Unit.prefab"
            };
            ...
            Assert.That(
                AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Lesson02/Prefabs/Cursor.prefab"),
                Is.Null);

            Assert.That(
                AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Lesson02/Prefabs/Tile.prefab"),
                Is.Null);

            for (int index = 0; index <= 6; index++)
            {
                Assert.That(
                    AssetDatabase.LoadAssetAtPath<Tile>($"Assets/Lesson02/Tiles/Strategy_{index}.asset"),
                    Is.Not.Null);
            }

            Button undoButton = GameObject.Find("Undo Button")?.GetComponent<Button>();
            Assert.That(undoButton, Is.Not.Null);
            Assert.That(undoButton.onClick.GetPersistentEventCount(), Is.Zero);
