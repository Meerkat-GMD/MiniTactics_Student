...
namespace MiniTactics.Lesson06
{
    ...
    public sealed class BoardView : MonoBehaviour
    {
        ...
        {
            if (selectedUnit == null || !selectedUnit.isActiveAndEnabled || !selectedUnit.IsAlive)
            {
                return MovementRangeResult.Empty(Vector2Int.zero);
            }

            Vector2Int start = WorldToCell(selectedUnit.transform.position);
            return MovementRange.Calculate(
                Board,
                start,
                selectedUnit.MoveBudget,
                GetOccupiedCells(selectedUnit));
        }

        public IReadOnlyCollection<Vector2Int> GetOccupiedCells(Unit movingUnit)
        {
            ...
            {
                if (unit != movingUnit && unit.IsAlive)
                {
        ...
        {
            if (selectedUnit == null || !selectedUnit.isActiveAndEnabled || !selectedUnit.IsAlive ||
                IsOccupiedByOther(selectedUnit, targetCell))
            {
                return false;
            }
