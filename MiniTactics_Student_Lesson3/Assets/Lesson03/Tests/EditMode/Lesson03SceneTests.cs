...
{
    public sealed class Lesson03SceneTests
    {
        private const string ScenePath = "Assets/Lesson03/Scenes/Lesson03.unity";

        [Test]
        public void Lesson03Scene_ContainsWiredClickMovementBoardUnitsAndCamera()
        {
            ...
            string[] prefabPaths =
            {
                "Assets/Lesson03/Prefabs/Unit.prefab"
            };
            ...
            Assert.That(
                AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Lesson03/Prefabs/Cursor.prefab"),
                Is.Null);

            Assert.That(
                AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Lesson03/Prefabs/Tile.prefab"),
                Is.Null);

            for (int index = 0; index <= 6; index++)
            {
                Assert.That(
                    AssetDatabase.LoadAssetAtPath<Tile>($"Assets/Lesson03/Tiles/Strategy_{index}.asset"),
                    Is.Not.Null);
            }
