...
namespace MiniTactics.Lesson04.Tests
{
    public sealed class BattleControllerTests
    {
        ...
        [Test]
        public void MovementRange_UsesTerrainCostsAndDoesNotBlockAtOccupiedCell()
        {
            ...
            Assert.That(range.TryGetCost(new Vector2Int(6, 4), out int occupiedCost), Is.True);
            Assert.That(occupiedCost, Is.EqualTo(1));
            Assert.That(range.TryGetCost(new Vector2Int(7, 4), out int beyondCost), Is.True);
            Assert.That(beyondCost, Is.EqualTo(2));
